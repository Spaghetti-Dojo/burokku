...
namespace SpaghettiDojo\Burokku\PackageName;
...
#### Example 1: Simple Feature (No Constants, No Configuration)

A feature that modifies WordPress defaults without additional configuration:
...
#### Example 2: Feature with Configuration and Asset Management

A feature that needs configuration data and handles asset enqueueing:
...
**Characteristics:**
- Constant for style handle (needed for asset management)
- PHPStan type annotation for complex array structure
- Constructor accepts configuration data
- Multiple WordPress hooks for different aspects
- Multiple private methods for different operations
...
|---------|-----------|---------|
| **Package namespace** | `SpaghettiDojo\Burokku\FeatureName` | `SpaghettiDojo\Burokku\BlockStyles` |
...
namespace SpaghettiDojo\Burokku\MyFeature;
...
class Module implements ExecutableModule
{
    ...
    public function run(ContainerInterface $container): bool
    {
        MyFeature::new('configuration')->init();
...
final readonly class MyFeature
{
    private const HANDLE = 'burokku-my-feature';

    public static function new(string $config): self
    {
    ...
    private function enqueue(): void
    {
        wp_enqueue_style(
            self::HANDLE,
            get_theme_file_uri('dist/styles/my-feature.css'),
            [],
            wp_get_theme()->get('Version')
        );
    }
