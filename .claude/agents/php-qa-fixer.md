---
name: "php-qa-fixer"
description: "Use this agent when you need to identify and fix PHPCS (Syde standard) and PHPStan (level 9) issues in the Burokku WordPress theme project. This agent should be used after writing or modifying PHP files, or when running `composer cs` or `composer analysis` reveals errors that need to be resolved.\n\n<example>\nContext: The user has just written a new PHP module and wants to ensure it passes quality checks.\nuser: \"I just finished writing the new BlockPatterns module, can you check it for issues?\"\nassistant: \"I'll launch the php-qa-fixer agent to analyze and fix any PHPCS and PHPStan issues in the new module.\"\n<commentary>\nSince new PHP code was written, use the Agent tool to launch the php-qa-fixer agent to run quality checks and fix issues.\n</commentary>\n</example>\n\n<example>\nContext: The user ran the QA suite and got errors.\nuser: \"composer cs is reporting a bunch of errors, can you fix them?\"\nassistant: \"I'll use the php-qa-fixer agent to analyze and resolve those PHPCS issues.\"\n<commentary>\nThe user has explicitly encountered PHP quality issues, so launch the php-qa-fixer agent to diagnose and fix them.\n</commentary>\n</example>\n\n<example>\nContext: The user is about to commit and wants a clean QA run.\nuser: \"I want to make sure the PHP code is clean before I commit.\"\nassistant: \"Let me use the php-qa-fixer agent to run the full PHP quality suite and fix any issues found.\"\n<commentary>\nPre-commit PHP quality validation is a primary use case for this agent.\n</commentary>\n</example>"
