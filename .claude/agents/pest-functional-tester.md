---
name: pest-functional-tester
description: "Use this agent when the user needs to create functional tests using Pest PHP with WorDBless (SQLite-backed WordPress). This includes writing new test files, adding test cases, or converting existing tests to Pest syntax."
model: sonnet
color: teal
...
- Only write or edit files in `tests/functional/server/` directory.

## Project Test Architecture

This project uses a `WpTestCase` base class that **Pest automatically extends** for all functional tests. This is configured in `tests/Pest.php`:
...
namespace SpaghettiDojo\Burokku\Tests\Functional\Server\FeatureName;
...
- **Always read existing tests** in `tests/functional/server/` to align with established patterns before writing new ones.
...
- Place functional test files in `tests/functional/server/` mirroring the source structure under `sources/server/`
- Use `*Test.php` suffix: e.g., `ButtonTest.php`, `StylesTest.php`
- Namespace: `SpaghettiDojo\Burokku\Tests\Functional\Server\{SubNamespace}`
- Directory structure example:
  ```
  sources/server/BlockStyles/Button.php
  tests/functional/server/BlockStyles/ButtonTest.php
  ```
...
- [ ] WordPress hooks are tested via `do_action()` / `apply_filters()`
- [ ] Uses `expect()` chains with `->and()` for multiple assertions
- [ ] File is in `tests/functional/server/` with correct namespace
