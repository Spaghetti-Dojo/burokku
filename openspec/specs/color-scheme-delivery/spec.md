...
Runtime delivery of both color schemes: the scoped CSS structure (`:root` vs `[data-theme="light"]`), the document `color-scheme` flag, enqueueing of the generated stylesheet, per-scheme translucent surfaces via `light-dark()`, toggle integration, and the CI drift guard.
