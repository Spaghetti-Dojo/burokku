...
The active editor scheme SHALL be applied by setting the `data-theme` attribute (`light` or
`dark`) on the canvas document, per the `theme-state` contract. When the canvas is rendered in an
iframe, the attribute SHALL be set on the iframe document (`iframe[name="editor-canvas"]`); when the
canvas is not iframed, it SHALL be set on the editor's own document. The attribute SHALL be set on
**both** the document root (`<html>`, for front-end parity and the `theme-toggle` block's
`:root[data-theme]` rules) **and** the `.editor-styles-wrapper` element, because WordPress re-emits
the theme.json preset custom properties on `.editor-styles-wrapper`; an override only on the
ancestor `<html>` is shadowed for content inside the wrapper and the light tokens do not resolve.
The capability SHALL NOT redefine the `data-theme` signal, its values, or the scoped custom
properties — those remain owned by `theme-state` and `color-scheme-delivery`.

#### Scenario: Attribute set on the iframe canvas

- **WHEN** the scheme is set to `light` and the canvas is iframed
- **THEN** the iframe document's `<html>` and `.editor-styles-wrapper` both carry
  `data-theme="light"`
- **AND** the canvas renders with the light custom properties already delivered by
